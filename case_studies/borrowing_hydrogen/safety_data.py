...
def get_safety_data(name):
    ...
    payload = {"response_type": "save", "response_basename": f"compound_CID_{cid}"}
    ...
    for section in res["Record"]["Section"]:
        if section["TOCHeading"] == "Safety and Hazards":
            ...
            try:
                sds["hazards"] = [
                    hazard["String"]
                    for hazard in hazards_section[2]["Value"]["StringWithMarkup"]
                ][1:-1]
                sds["exposure_limits"] = section["Section"][6]["Section"][0][
                    "Information"
                ][0]["Value"]["StringWithMarkup"][0]["String"]
                sds["reactivity"] = section["Section"][7]["Section"][3]["Information"][
                    0
