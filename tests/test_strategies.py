...
def test_nm1():
    ...
    initial_exp = pd.DataFrame(data={'temperature': [-0.409,-1.112,0.17], 'flowrate_a': [-3.424,0.33,-3.252]})   # initial experimental points
    ...
    for i in range(num_iter):
        ...
        if i == 0:
            next_experiments, xbest, fbest, res = strategy.suggest_experiments(prev_res=None)
        # runs with history
        else:
