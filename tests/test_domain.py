...
from summit.domain import (Variable, ContinuousVariable, DiscreteVariable,
...
def test_continuous_variable():
    ...
    bounds = [1, 100]
    var = ContinuousVariable(name='temperature', 
...
def test_discrete_variable():
    ...
    var = DiscreteVariable(name='reactant', 
                           description='aromatic reactant', 
                           levels=levels)
    assert isinstance(var, Variable)
    assert var.is_objective == False
    ...
    with pytest.raises(ValueError):
        var = DiscreteVariable(name='nu',
                               description='not_unique',
                               levels=levels)

    # Make sure exception raise if a list is not passed
    levels = ('benzene', 'toluene')
    with pytest.raises(TypeError):
        var = DiscreteVariable(name='nl',
...
def test_descriptors_variable():
    ...
    solvent_ds = DataSet([[5, 81],[-93, 111]], 
...
def test_domain():
    var1 = ContinuousVariable(name='temperature', 
                        description='reaction temperature in celsius', 
                        bounds=[30, 100],)
    var2 = ContinuousVariable(name='flowrate_a', 
                        description='flowrate of reactant a', 
                        bounds=[1, 100])
    var3 = ContinuousVariable(name='flowrate_b', 
                            description='flowrate of reactant b', 
                            bounds=[1, 100])
    var4 = DiscreteVariable(name='base', 
    ...
    var6 = ContinuousVariable(name='yield',
    ...
    domain = Domain(variables=[var1, var2, var3, var4, var5, var6], 
                    constraints=[c])
    ...
    assert domain.num_variables(include_outputs=True) == \
            new_domain.num_variables(include_outputs=True)
    assert domain.num_discrete_variables() == \
            new_domain.num_discrete_variables()
    assert domain.num_discrete_variables(include_outputs=True) == \
            new_domain.num_discrete_variables(include_outputs=True)
    assert domain.num_continuous_dimensions() ==  \
            new_domain.num_continuous_dimensions()
    assert domain.num_continuous_dimensions(include_outputs=True) == \
