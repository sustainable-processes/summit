...
class MIT_case5(Experiment):
    ...
    >>> results = exp5.run_experiments(conditions)
    ...
    def _setup_domain(self):
        ...
        domain += ContinuousVariable(name="conc_cat", description=des_1, bounds=[0.835*10**(-3),4.175*10**(-3)])

        des_2 = "reaction time"
        domain += ContinuousVariable(
            name="t", description=des_2, bounds=[60,600]
        )
    ...
    def _integrand(self, t, C, cat_index, T):
        ...
        if  T < 80+273.71:
            ...
            E_Ai = [-5.0+0.3*(T-80-273.71), 0.3, 0.3, 0.7, 0.7, 2.2, 3.8, 7.3]
        
        k = (
            lambda conc_cat, A, E_A, E_Ai, temp: 
            np.sqrt(conc_cat) * A * np.exp(-(E_A + E_Ai)/ (R * temp))
