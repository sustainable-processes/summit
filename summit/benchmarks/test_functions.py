...
class Hartmann3D(Experiment):
    ...
    def plot(self, **kwargs):
        ...
        expand_bounds = 0
        ax.axes.set_xlim3d(left=bounds_x_1[0] - expand_bounds, right=bounds_x_1[1] + expand_bounds)
        ax.axes.set_ylim3d(bottom=bounds_x_2[0] - expand_bounds, top=bounds_x_2[1] + expand_bounds)
        ax.axes.set_zlim3d(bottom=bounds_x_3[0] - expand_bounds, top=bounds_x_3[1] + expand_bounds)

        # plot evaluated and extra points with enumeration
        for c in range(len(points)):
            tmp_x_1, tmp_x_2, tmp_x_3, col = points[c][0], points[c][1], points[c][2], points[c][3]
            if self.maximize:
                col = -col
            p = ax.scatter(tmp_x_1, tmp_x_2, tmp_x_3, c=col, cmap='seismic',vmin=-4, vmax=0)
            ax.text(tmp_x_1 + .05, tmp_x_2 + .05, tmp_x_3 + .05, c+1, fontsize=7)
